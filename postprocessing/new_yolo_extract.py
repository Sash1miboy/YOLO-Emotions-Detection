...
class YOLOClassMetricsExtractor:
    ...
    def extract_accuracy_metrics(self, model, model_name: str, results_csv: Path):
        """Extract accuracy metrics & Training Time dari csv"""
        ...
        try:
            ...
            print(f"Gagal extract per-class data: {e}")
    ...
    def extract_model_info(self, model):
        ...
        if info is None:
            raise ValueError("ERROR: Info None")
        if not isinstance(info, (tuple, list)):
            raise TypeError("ERROR: Info format wrong")
        if len(info) < 4:
            raise ValueError("ERROR: Info length < 4")
    ...
    def save_results(self, output_dir: Path):
        ...
        print(f"\nSummary saved ke: {output_dir / 'yolo_metrics_summary.csv'}")
        ...
        if self.detailed_results:
            ...
            print(f"Detailed saved ke: {output_dir / 'yolo_metrics_detailed.csv'}")
        ...
        try:
            ...
            with pd.ExcelWriter(excel_path, engine="openpyxl") as writer:
                ...
                if self.detailed_results:
                    df_detail.to_excel(  # type: ignore
                        writer,
                        index=False,
