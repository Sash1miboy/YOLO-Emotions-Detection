...
class YOLOClassMetricsExtractor:
    ...
    def extract_accuracy_metrics(self, model, model_name: str, results_csv: Path):
        ...
        test_params = {
            "imgsz": 640, "batch": 32, "save_json": True, "device": 0,
            "plots": False, "split": 'test', "verbose": True, 
    ...
    def extract_efficiency_metrics(self, model, model_name: str, num_samples: int = 100):
        ...
        if len(test_images) == 0: raise FileNotFoundError(f"Gak ada image di {test_images_dir}")
        
        test_images = test_images[:num_samples]
        
        print(f"🔥 Warming up GPU...")
        for _ in range(10): _ = model.predict(test_images[0], verbose=False)
        
        print(f"Benchmarking dengan {len(test_images)} images...")
        preprocess_times, inference_times, postprocess_times = [], [], []
        
        predict_params = {"imgsz": 640, "device": 0, "batch": 1, "verbose": False, "stream": False}
        ...
        if not preprocess_times: raise RuntimeError("No timing data collected")
    ...
    def extract_model_info(self, model):
        ...
        if info is None: raise ValueError("❌ FATAL: Info None")
        if not isinstance(info, (tuple, list)): raise TypeError("❌ FATAL: Info format wrong")
        if len(info) < 4: raise ValueError("❌ FATAL: Info length < 4")
    ...
    def process_single_model(self, model_info: dict, benchmark_samples: int = 100):
        ...
        # Extract Efficiency
        eff_metrics = self.extract_efficiency_metrics(model, model_name, num_samples=benchmark_samples)
    ...
    def save_results(self, output_dir: Path):
        ...
        first_cols = ['Model', 'Version', 'Size', 'Parameters (M)', 'GFLOPs', 'Training Time', 'mAP50-95', 'FPS', 'Efficiency Score']
        cols = first_cols + [c for c in df_summary.columns if c not in first_cols]
        df_summary = df_summary[cols]
        
        df_summary.to_csv(output_dir / "yolo_metrics_summary.csv", index=False, float_format='%.4f')
        ...
        if self.detailed_results:
            ...
            df_detail.to_csv(output_dir / "yolo_metrics_detailed.csv", index=False, float_format='%.4f')
        ...
        try:
            ...
            with pd.ExcelWriter(excel_path, engine='openpyxl') as writer:
                df_summary.to_excel(writer, index=False, sheet_name='Summary', float_format='%.4f')
                if self.detailed_results:
                    df_detail.to_excel(writer, index=False, sheet_name='Per-Class Details', float_format='%.4f') # type: ignore
